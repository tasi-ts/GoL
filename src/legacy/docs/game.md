...
1. If `rand_rate` is truthy, `board.add_random_coords(rate=rand_rate)`.
2. Print initial board and area.
3. Loop `i` in `0 .. max_iter-1`:
   - `advance_board()`
   - Print generation line.
   - If `not check_if_changed()`, append current board and **break**.
4. Commented-out hooks exist for per-generation printing and `display_board`.

With default `rand_rate=0.5`, seeding always runs. Setting `rand_rate=0` would skip random fill (falsy), leaving an empty board unless you pre-seed elsewhere.
...
Then builds a matplotlib figure and loops over `game.sequence`:

- Clears axes, `imshow` each generation's `convert_to_binary_image()`, gray colormap.
- Title: `Generation {i}.`
- `plt.pause(0.075)` between frames.

Also prints `sys.executable` at startup (useful to confirm which Python interpreter is running).
...
- **`copy.deepcopy`** — generation snapshots
- **`matplotlib.pyplot`** — animation after simulation
- **`sys`** — interpreter path in `main()`

See [`legacy_requirements.txt`](../legacy_requirements.txt).
...
- Save `sequence` to disk or GIF without interactive matplotlib
